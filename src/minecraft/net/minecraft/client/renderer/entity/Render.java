...
import net.minecraft.afyaan.modules.NameTag;
import net.minecraft.afyaan.utils.FriendUser;
...
public abstract class Render
{
    ...
    protected void func_147906_a(Entity p_147906_1_, String p_147906_2_, double p_147906_3_, double p_147906_5_, double p_147906_7_, int p_147906_9_)
    {
        ...
        if (var10 <= (double)(p_147906_9_ * p_147906_9_))
        {
        	...
        	double dis = p_147906_1_.getDistance(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ);
        	FontRenderer var12 = this.getFontRendererFromRenderManager();
            float var13 = 1.6F;
            float var14 = 0.016666668F * var13;
            if(p_147906_1_ instanceof EntityPlayer){
            	if(McCheatClient.getModuleManager().findMod(NameTag.class).isEnabled()){
                	if(dis > 10){
                		var14 = 0.016666668F * (float)dis / 8;
                	}
                	if(dis > 20){
                		var14 = 0.016666668F * (float)dis / 7;
                	}
            ...
            if((p_147906_1_ instanceof EntityPlayer)){
            if(McCheatClient.getModuleManager().findMod(NameTag.class).isEnabled() == false){
	            GL11.glDisable(GL11.GL_TEXTURE_2D);
            	...
            	float width = var12.getStringWidth(p_147906_2_) + 2;
            	float height = 10;
            	float x1 = width / 2;
            	float y1 = height / 2 + (float)3.5;
            	GuiUtils.drawRect(x1, y1, x1 - width, y1 - height -2 , 0xff000000);
            	GuiUtils.drawRect(x1, y1, x1 - width - 2, y1 - height , 0xff000000);
            	GuiUtils.drawRect(x1, y1 + 2, x1 - width, y1 - height , 0xff000000);
            	GuiUtils.drawRect(x1 + 2, y1, x1 - width, y1 - height , 0xff000000);
            	GuiUtils.drawRect(x1, y1, x1 - width, y1 - height, 0xffc41159);
            	var12.drawString(p_147906_2_, -var12.getStringWidth(p_147906_2_) / 2, var16, -1);
            	var12.drawString(p_147906_2_, -var12.getStringWidth(p_147906_2_) / 2, var16, 553648127);
	            ...
	            var15.startDrawingQuads();
	            int var17 = var12.getStringWidth(p_147906_2_) / 2;
	            var15.setColorRGBA_F(255.0F, 0.0F, 0.0F, 0.25F);
	            if(p_147906_1_ instanceof EntityPlayer){
	        		
	            	
	            	EntityLivingBase elb = (EntityLivingBase)p_147906_1_;
	            	Entity entity = p_147906_1_;
	            	
	                
	                String username = elb.func_145748_c_().getFormattedText().replaceAll("§r", ""); 
	            ...
	            for(FriendUser fu : FriendsUtils.friendsList){
	            	if((p_147906_1_ instanceof EntityPlayer)){
		            	...
		            	if(username.equalsIgnoreCase(fu.nick)){
		            //System.out.println(elb.func_145748_c_().getFormattedText());
		            		var15.setColorRGBA_F(0.0F, 255.0F, 0.0F, 0.25F);
		            	}
	            	}
	            }
	            }
            	...
            	Entity entity = p_147906_1_;
		            ...
		            for (varM = 0; varM < 4; ++varM)
		        	{
		        		...
		        		if(entity.getLastActiveItems()[varM] != null)
		        		{
							ri.renderItemAndEffectIntoGUI(Minecraft.getMinecraft().fontRenderer, Minecraft.getMinecraft().getTextureManager(), entity.getLastActiveItems()[varM], x, y - 40);
				            ri.renderItemOverlayIntoGUI(Minecraft.getMinecraft().fontRenderer, Minecraft.getMinecraft().getTextureManager(), entity.getLastActiveItems()[varM], x, y - 40);
		        		}
		        		x -= 15;
					}
		            if(elb.getHeldItem() != null){
		            ri.renderItemAndEffectIntoGUI(Minecraft.getMinecraft().fontRenderer, Minecraft.getMinecraft().getTextureManager(), elb.getHeldItem(), x, y - 40);
		            ri.renderItemOverlayIntoGUI(Minecraft.getMinecraft().fontRenderer, Minecraft.getMinecraft().getTextureManager(), elb.getHeldItem(), x, y - 40);
