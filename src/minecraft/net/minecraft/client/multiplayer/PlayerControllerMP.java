...
import net.minecraft.afyaan.modules.CentrumPvpMode;
...
public class PlayerControllerMP
{
    ...
    public Entity attackEntity2 = null;
    ...
    public void attackEntity(EntityPlayer par1EntityPlayer, Entity par2Entity)
    {
    	...
    	if((par2Entity instanceof EntityPlayer)){
        	attackEntity2 = par2Entity;
        	McCheatClient.userattackEntity = (Entity)par2Entity;
        }
    	
    	AttackEntity__(par1EntityPlayer, par2Entity);
    }

    public void AttackEntity__(EntityPlayer par1EntityPlayer, Entity par2Entity){
    	if(par2Entity instanceof EntityPlayer){
    		
	    	EntityLivingBase elb = (EntityLivingBase)par2Entity;
	    	String nick = elb.func_145748_c_().getFormattedText().replaceAll("§r", "");
	    	if(ModuleManager.findMod(CentrumPvpMode.class).isEnabled()){
	    		double var2 = (double)this.mc.playerController.getBlockReachDistance();
	    		Vec3 var6 = this.mc.renderViewEntity.getPosition(1.0f);
	    		Vec3 var7 = this.mc.renderViewEntity.getLook(1.0f);
	            ...
	            if(var11.size() > 0){
	    			for (int var144 = 0; var144 < var11.size(); ++var144)
	                {
	                	Entity var155 = (Entity)var11.get(var144);
	                	if(elb.getCommandSenderName().equalsIgnoreCase(var155.getCommandSenderName())){
	                		System.out.println(var155.getCommandSenderName());
	                		attackEntity_(par1EntityPlayer, par2Entity);
	                	}else{
	                		return;
	                	}
	                }
    ...
    public void attackEntity_(EntityPlayer par1EntityPlayer, Entity par2Entity){   	
    	{
			this.syncCurrentPlayItem();
